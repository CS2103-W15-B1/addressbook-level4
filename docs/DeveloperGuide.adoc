...
This section details the implementation of the Rubrika's `RemoveTag` feature.
...
If the user inputs `all` in the `INDEX` field, instead of removing a tag based on the list displayed in `Model`, the tag is removed from everyone in the `UniquePersonList` of `AddressBook`. The `TAG` is then deleted from the `UniqueTagList` in `AddressBook`. +

If the user inputs an `Integer` in the `INDEX` field, the tag is removed from the person as well as the `UniqueTagList` in the `AddressBook`. Thereafter, we re-sync the `Tag` Objects in `UniqueTagList` with all the `Tag` contained within the `Person` Object in the `UniquePersonList`. +
...
**Aspect:** How to sync `tag` in `UniqueTagList` after deleting from one `Person`

**Alternative 1 (current):** Deletes `TAG` from `UniqueTagList`, then re-sync with `persons` list. +
**Pros:** Easier to implement. +
**Cons:** If the `Tag` still exists in another contact, then it is a pointless deletion.

**Alternative 2:** Checks if 'Tag` deleted is contained in another `Person`. +
**Pros:** Not required to delete `Tag` from `UniqueTagList` if it still exist in any `Person`. +
**Cons:** Uses more resources since Rubrika will need to check check through everyone before deleting from `UniqueTagList`.
// end::rmtag[]

// tag::sort[]
=== Sort Function

This section details the implementation of the Rubrika's `Sort` feature.
...
**Pros:** A more defensive approach as `UniquePersonList` in `AddressBook` is not modified in anyway. +
...
**Alternative 2:** Sorts the `UniquePersonList` found in `AddressBook`. +
**Pros:** Easier to implement since `UniquePersonList` is implemented using `List`. +
...
This section details the implementation of the Rubrika's `Import` feature.
...
**Pros** : Less coupling as the parser will only be associated with `ImportCommandParser`. +
**Cons** : Very intensive to implement within the given time frame.

=== Export Function

This section details the implementation of the Rubrika's `Export` feature.

The export function is done by the `ExportCommand` class, and currently supports exporting to vCard files (.vcf) and XML files (.xml).

[NOTE]
Currently, the export function only ensures that the vCard file created is supported by Rubrika application.

After input validation in the `ExportCommandParser` class, the file path is passed to the `ExportCommand` class.

[NOTE]
If the file format is not specified by the user during input, ParseException will be thrown before execution of the Command.
...
The following Sequence Diagram will detail the events that occur after a user has entered a valid export Command for a XML file.

image::ExportSequenceDiagram.png[width="800"]
_Figure 4.8.1 : Sequence Diagram of export command._
...
**Aspect**: Interoperability of vCards. +

**Alternative 1 (current choice)**: vCards created by Rubrika are only expected to be readable by Rubrika. +
**Pros:** Relatively easier to implement, as long as the `Import` function is able to take in the fields created by the `Export` function. +
**Cons:** Export file might not work on other applications.

**Alternative 2**: Allow vCard exported to be used universally. +
**Pros:** More usability of vCard export. +
**Cons:** Extremely difficult to implement as vCards have many variation of formats across different application.
...
image::FindTagComponentSequenceDiagram.png[width="800"]
_Figure 4.9.1 : Sequence Diagram of FindTag function_
[NOTE]
