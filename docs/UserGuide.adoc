...
Option to remove a tag for selected people available since V1.2
...
Currently, Rubrika allows you to list your clients by `FIELD` (*name, email or phone*) following an `ORDER` (*[asc]* ascending, *[dsc]* descending). +

To re-arrange your contact list follow these steps.
...
Format: `import` or `import FILENAME`
...
=== Autocomplete Anywhere in the Command Input Box
